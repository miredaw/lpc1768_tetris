...
* Attention		 : None 
*******************************************************************************/
static void __attribute__((optnone)) wait_delay(int count)
{
	while(count--);
}
...
* Attention		 : None
*******************************************************************************/
__attribute__((always_inline)) void LCD_WriteIndex(uint16_t index)
{
	LCD_CS(0);
	LCD_RS(0);
	LCD_RD(1);
...
* Attention		 : None
*******************************************************************************/
__attribute__((always_inline)) void LCD_WriteData(uint16_t data)
{				
	LCD_CS(0);
	LCD_RS(1);   
	LCD_Send( data );
...
* Attention		 : None
*******************************************************************************/
static void __attribute__((optnone)) delay_ms(uint16_t ms)    
{ 
	uint16_t i,j; 
	for( i = 0; i < ms; i++ )
	{ 
...
* Attention		 : None
*******************************************************************************/
__attribute__((optnone)) void LCD_Initialization(void)
{
	uint16_t DeviceCode;
	...
	else if (DeviceCode == 0x8999)  // <--- ADD THIS BLOCK
	{
    LCD_Code = SSD1289;
    LCD_WriteReg(0x0000,0x0001);    delay_ms(15);
    LCD_WriteReg(0x0003,0xA8A4);    delay_ms(15);
    LCD_WriteReg(0x000C,0x0002);    delay_ms(15);
    LCD_WriteReg(0x000D,0x000A);    delay_ms(15);
    LCD_WriteReg(0x000E,0x2C00);    delay_ms(15);
    LCD_WriteReg(0x001E,0x00B8);    delay_ms(15);
    LCD_WriteReg(0x0001,0x2B3F);    delay_ms(15); // Drive Output Control
    LCD_WriteReg(0x0002,0x0600);
    LCD_WriteReg(0x0010,0x0000);
    LCD_WriteReg(0x0011,0x6030); // Entry Mode
    LCD_WriteReg(0x0005,0x0000);
    LCD_WriteReg(0x0006,0x0000);
    LCD_WriteReg(0x0016,0xEF1C);
    LCD_WriteReg(0x0017,0x0003);
    LCD_WriteReg(0x0007,0x0233); // Display Control
    LCD_WriteReg(0x000B,0x5312);
    LCD_WriteReg(0x000F,0x0000);
...
const int8_t tetromino_shapes[7][4][4][2] = {
    // I-piece (Cyan) - TETRO_I
    {
        {{0,1},{1,1},{2,1},{3,1}},  // ROT_0 - Horizontal
        {{2,0},{2,1},{2,2},{2,3}},  // ROT_90 - Vertical
        {{0,2},{1,2},{2,2},{3,2}},  // ROT_180 - Horizontal
        {{1,0},{1,1},{1,2},{1,3}}   // ROT_270 - Vertical
    },
    // O-piece (Yellow) - TETRO_O
    {
        {{1,0},{2,0},{1,1},{2,1}},  // ROT_0
        {{1,0},{2,0},{1,1},{2,1}},  // ROT_90
        {{1,0},{2,0},{1,1},{2,1}},  // ROT_180
        {{1,0},{2,0},{1,1},{2,1}}   // ROT_270
    },
    // T-piece (Magenta) - TETRO_T
    {
        {{1,0},{0,1},{1,1},{2,1}},  // ROT_0
        {{1,0},{1,1},{2,1},{1,2}},  // ROT_90
        {{0,1},{1,1},{2,1},{1,2}},  // ROT_180
        {{1,0},{0,1},{1,1},{1,2}}   // ROT_270
    },
    // S-piece (Green) - TETRO_S
    {
        {{1,0},{2,0},{0,1},{1,1}},  // ROT_0
        {{1,0},{1,1},{2,1},{2,2}},  // ROT_90
        {{1,1},{2,1},{0,2},{1,2}},  // ROT_180
        {{0,0},{0,1},{1,1},{1,2}}   // ROT_270
    },
    // Z-piece (Red) - TETRO_Z
    {
        {{0,0},{1,0},{1,1},{2,1}},  // ROT_0
        {{2,0},{1,1},{2,1},{1,2}},  // ROT_90
        {{0,1},{1,1},{1,2},{2,2}},  // ROT_180
        {{1,0},{0,1},{1,1},{0,2}}   // ROT_270
    },
    // J-piece (Blue) - TETRO_J
    {
        {{1,0},{1,1},{0,2},{1,2}},  // ROT_0
        {{0,0},{0,1},{1,1},{2,1}},  // ROT_90
        {{1,0},{2,0},{1,1},{1,2}},  // ROT_180
        {{0,1},{1,1},{2,1},{2,2}}   // ROT_270
    },
    // L-piece (Orange) - TETRO_L
    {
        {{1,0},{1,1},{1,2},{2,2}},  // ROT_0
        {{0,1},{1,1},{2,1},{0,2}},  // ROT_90
        {{0,0},{1,0},{1,1},{1,2}},  // ROT_180
        {{2,0},{0,1},{1,1},{2,1}}   // ROT_270
    }
};

void PutTetrominoes(uint16_t Xpos, uint16_t Ypos, uint8_t shape_id, uint8_t rotation, uint8_t block_size, uint16_t blockColor, uint16_t bkColor) {
    uint8_t i, j, k;
    
    if (shape_id > 6 || rotation > 3) return;
    
    // Draw each of the 4 blocks
    for (k = 0; k < 4; k++) {
        int8_t block_x = tetromino_shapes[shape_id][rotation][k][0];
        int8_t block_y = tetromino_shapes[shape_id][rotation][k][1];
        
        uint16_t start_x = Xpos + block_x * block_size;
        uint16_t start_y = Ypos + block_y * block_size;
        
        // Draw filled block
        for (i = 0; i < block_size; i++) {
            for (j = 0; j < block_size; j++) {
                if (start_x + i < MAX_X && start_y + j < MAX_Y) {
                    LCD_SetPoint(start_x + i, start_y + j, blockColor);
                }
            }
        }
    }
